...
class Field:
    ...
    def print_state(self,):
        print('You look araound and see')
    ...
    def gen_random():
        ...
        if rand == 0:
            return Field([])
        if rand == 1:
            return Field([monster(), monster()])
        if rand == 2:
            return Field([fighter(), fighter(), monster()])
...
class Map:
    ...
    def forward(self):
        if self.x == len(self.state) -1:
            print('You see huge mountains, which u can´t pass')
        else:
            self.x = self.x+1

    def backwards(self):
        if self.x == 0:
            print('You see cliffs, but u can´t jump safely')
        else:
            self.x = self.x -1
...
def quit_game(p, m):
    print('You commit suicide an leave this world.')
    exit(0)
...
if __name__ == '__main__':
   ...
   map = Map(5,5)
   print('(type help to list the commands availavle)\n')
   while True:
       command = input('>').lower().split(' ') #pickup axe
       if command[0] in Commands:
           Commands[command[0]](p, map)
       else:
           print('You run round in circles an don`t know what to do.')
       map.print_state()
